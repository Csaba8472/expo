...
| `EXPO_QR_SEXTANTS`                  | **boolean** | Force or disable compact (sextant) QR code rendering in the terminal. When not set, auto-detection based on terminal emulator is used. Set to `0` or `false` if your terminal font does not support Unicode sextant block characters (U+1FB00 range), causing broken QR codes.                                                                                                                                                  |
